...
describe('Remove Permission Usecase Test Suites', () => {
  ...
  beforeEach(async () => {
    ...
    usecase = new RemovePermissionUsecase(
      mockPermRepository,
      mockAdminRepository,
    );
    jest.clearAllMocks();
  ...
  it('should throw PermissionAlreadySetException if admin does not have the permission to be removed', async () => {
    jest
      .spyOn(mockAdminRepository, 'findById')
      .mockResolvedValueOnce(mockAdmin as any);
  ...
  it('should remove permission from admin', async () => {
    jest
      .spyOn(mockAdminRepository, 'findById')
      .mockResolvedValueOnce(mockAdmin as any);
    jest.spyOn(mockPermRepository, 'hasPermission').mockResolvedValueOnce(true);
    ...
    expect(result).toEqual({
      ...
      removed_permission: data.permission,
    });
  });
});
