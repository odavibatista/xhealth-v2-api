...
import { AdminLoginRequestDTO, AdminLoginResponseDTO } from "../../domain/dtos/requests/AdminLogin.request.dto";
...
export class AdminLoginUsecase implements UseCaseInterface  {
      ...
      async execute(data: AdminLoginRequestDTO): Promise<AdminLoginResponseDTO | InvalidCredentialsException | UnprocessableDataException> {
        const admin = await this.userRepository.findByEmail(data.email);

        if (!admin || !admin.id_administrator || !admin.name) throw new InvalidCredentialsException();
