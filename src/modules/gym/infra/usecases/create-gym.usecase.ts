...
import { validateAddress, validatePhone } from "../../../../shared/infra/utils/functions/validators";
...
export class CreateGymUsecase implements UseCaseInterface {
      ...
      async execute(data: CreateGymBodyDTO, administrator_id: string): Promise<CreateGymResponseDTO | UnauthorizedException | AccountNotFoundException> {
            validateAddress({
        ...
        const adminExists = await this.administratorRepository.findById(administrator_id);

        if (!adminExists) 
          throw new AccountNotFoundException();

        const hasPermission = await this.adminPermissionsRepository.hasPermission(administrator_id, 'can_create_gyms');

        if (!hasPermission) throw new UnauthorizedException('Você não tem permissão para criar academias.');
        ...
        if (!gym || !gym.id_gym || !gym.address_id) throw new UnauthorizedException('Erro ao criar academia.');
