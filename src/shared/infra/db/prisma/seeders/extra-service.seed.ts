  ...
  const extraServices = [
    {
      ...
      description: encrypterProvider.encrypt({
        content: 'Treinamento personalizado com acompanhamento profissional.',
      }),
      price: encrypterProvider.encrypt({ content: '99.90' }),
      imageUrl: encrypterProvider.encrypt({
        content: 'https://example.com/personal-trainer.jpg',
      }),
    ...
    },
    {
      ...
      description: encrypterProvider.encrypt({
        content: 'Consultoria nutricional para otimização de performance.',
      }),
      price: encrypterProvider.encrypt({ content: '79.90' }),
      imageUrl: encrypterProvider.encrypt({
        content: 'https://example.com/nutrition.jpg',
      }),
      created_by: existingAdmin.id_administrator,
    },
    {
      ...
      description: encrypterProvider.encrypt({
        content: 'Análise completa da composição corporal e desempenho físico.',
      }),
      price: encrypterProvider.encrypt({ content: '49.90' }),
      imageUrl: encrypterProvider.encrypt({
        content: 'https://example.com/physical-assessment.jpg',
      }),
      created_by: existingAdmin.id_administrator,
    },
    {
      ...
      description: encrypterProvider.encrypt({
        content: 'Apoio psicológico para saúde mental e emocional.',
      }),
      price: encrypterProvider.encrypt({ content: '89.90' }),
      imageUrl: encrypterProvider.encrypt({
        content: 'https://example.com/psychotherapy.jpg',
      }),
      created_by: existingAdmin.id_administrator,
    },
  ];
